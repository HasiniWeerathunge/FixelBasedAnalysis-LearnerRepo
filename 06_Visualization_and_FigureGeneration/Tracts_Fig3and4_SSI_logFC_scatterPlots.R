...
############################################################
# 4. Merge + clean + group definition (MATCH CLUSTER SCRIPT)
############################################################

data_final <- all_long %>%
  left_join(demo, by = "Subject") %>%
  mutate(
    SSIRater1 = as.numeric(SSIRater1),
...
############################################################
# 5. ROI selection (UNCHANGED LOGIC)
############################################################
...
############################################################
# 6. Loop over tracts (NOW MATCHES CLUSTER STYLE)
############################################################
...
for (tr in valid_tracts) {
  ...
  if (all(is.na(tr_data$SSIRater1))) next
  
  ############################################################
  # Residualize Mean_logFC (SAME AS CLUSTER SCRIPT)
  ############################################################
  ...
  ############################################################
  # Group-wise regression (MATCH CLUSTER STYLE)
  ############################################################
  ...
  ############################################################
  # Plot (NOW MATCHES CLUSTER STYLE EXACTLY)
  ############################################################
  
  p <- ggplot(tr_data, aes(x = SSIRater1, y = Mean_logFC_resid, color = Group)) +
  ...
  p <- p +
    geom_text(
      data = reg_stats,
      aes(
        x = max(tr_data$SSIRater1, na.rm = TRUE),
        y = y_pos,
        label = label,
        color = Group
      ),
